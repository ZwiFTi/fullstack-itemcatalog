...
def CatalogItemJSON(catalog_name, catalogitem_name):
    ...
    item = session.query(CatalogItem).filter_by(catalog_id = selectedCatalog.id, name=catalogitem_name).one()
...
def Details(catalog_name, catalogitem_name):
    ...
    return render_template('item.html', catalog = selectedCatalog, item = item, islogin = isLogin)
...
def newCatalogItem(catalog_name):
    ...
    if request.method == 'POST':
        ...
        newItem = CatalogItem(
            name=request.form['name'], description=request.form['description'], catalog=selectedCatalog, user=login_session['username'])

        if newItem.name not in names:
            session.add(newItem)
            session.commit()
            flash('Item named ' + request.form['name'] + ' created')
            return redirect(url_for('ListItems', catalog_name=catalog_name, islogin = isLogin))
        flash('Item already exists')
        return redirect(url_for('Index'))
    else:
        return render_template('newCatalogItem.html', catalog_name=catalog_name, islogin = isLogin)

@app.route('/catalog/<catalogitem_name>/edit', methods=['GET', 'POST'])
@login_required
def EditCatalogItem(catalogitem_name):
    if request.method == 'POST':
        selectedCatalogItem = session.query(CatalogItem).filter_by(name=catalogitem_name).one()
        print(selectedCatalogItem.description)
        if request.form['description'] != selectedCatalogItem.description:
            flash('Set items description from ' + selectedCatalogItem.description + ' to ' + request.form['description'] + '.')
        ...
        if request.form['name'] != catalogitem_name:
            flash('Set items name from ' + catalogitem_name + ' to ' + request.form['name'] + '.')


        return redirect(url_for('Details', catalog_name=selectedCatalogItem.catalog.name, catalogitem_name=selectedCatalogItem.name, isLogin = True))
    else:
        return render_template('editCatalogItem.html', catalogitem_name=catalogitem_name, islogin = True)
...
def DeleteCatalogItem(catalogitem_name):
    if 'username' in login_session:
        ...
        return redirect(url_for('showLogin'))
    if request.method == 'POST':
        itemToDelete = session.query(CatalogItem).filter_by(name=catalogitem_name).first()
        session.delete(itemToDelete)
        session.commit()
        flash('deleted item ' + catalogitem_name + '.')
        return redirect(url_for('Index'))
    else:
        return render_template('deleteCatalogItem.html', catalogitem_name=catalogitem_name, islogin=isLogin)
